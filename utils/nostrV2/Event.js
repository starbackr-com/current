...
const parseContent = (message) => {
  let imageRegex =
    /(http(s?):)([\/|.|\w|\s|\-|_])*\.(?:jpg|gif|png|jpeg|webp)/g;
...
export class Event {
  ...
  saveBadge() {
    const { id, pubkey, created_at, kind, tags, sig } = this;
    try {
      const badge = {
        id,
        pubkey,
        ...
        tags,
        sig,
        badgeDefinitions: tags.filter(tag => tag[0] === 'a')
      };
      store.dispatch(setUserBadges(badge));
    } catch (err) {
      console.log(err);
    }
  }
